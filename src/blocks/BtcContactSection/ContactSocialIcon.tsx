...
const resolveNetwork = (label: string, url: string) => {
...
export const ContactSocialIcon: React.FC<Props> = ({ label, url }) => {
  const network = resolveNetwork(label, url)

  return (
    <a
      aria-label={label}
      className="flex h-12 w-12 items-center justify-center rounded-xl bg-graphite-100 text-graphite-600 transition-colors hover:bg-solar-400 hover:text-graphite-900"
      href={url}
